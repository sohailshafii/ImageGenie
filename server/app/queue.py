...
def publish_next(topic_id: str, uid: str, variant: str = DEFAULT_VARIANT) -> str:
    ...
    (app/artifact_keys.py). The default variant writes no `variant` field at all,
    so an ordinary job's payload is byte-identical to what it was before variants
    existed — messages already in flight during a deploy stay valid.
    """
    payload = {"uid": uid}
    if variant != DEFAULT_VARIANT:
        payload["variant"] = variant
    return publish_json(_publisher(), topic_id, payload)
